                ...
                <a :href=" 'https:' +  item.url  " target="_blank">
                    <div class="list-content">
                        ...
                        <div class="list-info">
                            ...
                            <p class="list-price">
                                <span><b>{{ item.price }}</b> <i class="history-lowest" v-if="item.isHistoryLowest">历史最低</i> <i class="coupon" v-if="item.coupon">{{ item.coupon }}元券</i></span>
                            </p>
                            <p class="list-source">
...
<script>
    ...
    export default {
        ...
        methods: {
            getDataOfChina() {
                this.axios.get(this.apiUrl + 'api/popupPage/getListOfMyFocus?page=1&size=' + this.size).then((res) => {
                    for (var i = 0; i < res.data.data.productList.length; i++) {
                        res.data.data.productList[i].price = $.trim(res.data.data.productList[i].price);
                ...
                $('.HomePage').on('scroll', function() {
                    ...
                    if (scrollTop + wrapper >= ulHeight - 10) { //监控滚动条滚动到底部
                        if (!_this.busy) {
                            _this.busy = true;
                            setTimeout(function() {
                                _this.axios.get(_this.apiUrl + 'api/popupPage/getListOfMyFocus?page=' + _this.page + '&size=' + _this.size).then((res) => {
                                    //更新数组列表
            ...
            .list-content {
                ...
                .list-price {
                    ...
                    i {
                        ...
                        margin-top: 10px;
                        padding: 2px 6px;
                        background: linear-gradient(to right, #FF4443, #FF8747);
                        border-radius: 4px;
                        color: #fff;
                        margin-right: 10px;
                        cursor: pointer;
